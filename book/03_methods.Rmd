...
h_i(t|\textbf{x(t)}_i, Z_{ij}) = h_0(t) \exp(\boldsymbol\beta^T \textbf{x(t)}_i + b_i^t z_ij)
...
There have been numerous attempts to improve the efficiency of this simple algorithm. The largest inefficiencies arise in two areas in two areas. First, the tails of the distribution require a large amount of iterations to accurately map. Second, distributions with several local optima requires a greater amount of iterations for the algorithm to escape these minima/maxima. Many of the attempts to improve the efficiency of this algorithm have tried to address these points. The most recent is Hamiltonian Monte Carlo (HMC), which was codified into the Stan statistical software package\cite{Carpenter2016}. The key difference between what was described above and HMC, is the proposal distribution\cite{Kruschke2015}. 
