...
Baseline hazard estimation is restricted to only include parametric forms. It is incredibly common to not make parametric assumptions about the baseline hazard.
There is a large body of research into non-parametric baseline hazard estimation. 

If contrasting the effect of covariates in the predominant interest in the model, semi-parametric or non-parametric methods often serve as better models for such a task. 


Likelihood in the presence of right-censoring.


$$ \mathcal {L}(\theta|Y_i, d_i) = S(Y_i)^{d_i} f(Y_i)^{(1 - d_i)}$$ 



A key approach to modeling will be to make use of a parametric baseline hazard function. 




$$ {\displaystyle \arg \max _{\theta }{\mathcal {L}}(\theta |\textbf{x}_{i})=\arg \max _{\theta }f(\textbf{x}_{i}|\theta )} $$
...
The introduction of the Frailty also complicates estimation. The 
