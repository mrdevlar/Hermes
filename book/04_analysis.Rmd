...
This entire process is easily implemented in Stan as it can be directly computed in the `generated quantities` block\cite{Vehtari2014}. As with all information criterion metrics, the lower the value the less deviation there is from the 'true' data generating distribution. 
...
To generate the WAIC or LOO metrics, a point-wise log likelihood, `log_lik`, is required. The `generated quantities{}` block allows for these likelihoods to be extracted upon completion of sampling. The functions `WAIC()` and `LOO()` from the `rethinking` package can be used to compute the WAIC and LOO value for the model under the assumption that the `log_lik` vector is included in the output. 
