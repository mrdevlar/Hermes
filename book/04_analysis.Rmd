...
This section provides the basis for evaluating performance of time-to-event models for predictive outcomes. It begins by discussing the difficulties that arise from predicting time-to-even models. It then introduces the C-index and Widely-Applicable Information Criterion (WAIC) are discussed for model discrimination and calibration. Finally, a general heuristic is provided for evaluating model performance in this class of models.
<!-- Insert content sentences -->

Conventional methods for assessing performance encounter problems with time-to-event models. Data structure creates difficulties, both in terms of its sequential structure as well as the balance between events and non-events. 

Conventional methods of evaluating out of sample performance include some form of cross-validation. To escape the prospect of a model that has simply memorized the data, or overfit, a measure that examines data that was not included during the fitting of the model is used to assess performance. This out-of-sample data is essential 
...
It is not possible to perform a conventional k-fold cross-validation in this context. The removal of a random selection of data points from the sequence would result in underfitting and instability in the evaluation process. 

Due to censoring, the data is generally severely unbalanced. Very few industrial settings feature an abundance of failures. Decades of reliability engineering has ensured that most industrial systems are not prone to widespread failure. Photovoltaic inverters are not excluded from this generalization. As a result, the majority of observations will be non-events. While all observations may eventually become events, as all systems eventually break down, it is highly unlikely in a production setting that the data will be balanced. Total observations that are composed of 95% or higher non-event are common. This leads to difficulties in assessing the predictive accuracy of model, in balanced terms. Simply predicting all observations as non-events would lead to high accuracy. As a result some degree of costs have to be involved in the accuracy process. 
