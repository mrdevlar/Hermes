...
Time-to-event models focused on predictive outcomes create a unique problem in evaluating model performance. 

On one hand, the output of the model creates hazards, which are continuous and have a support on all positive real numbers $[0, \infty)$. These are always monotonically related to expected residual lifetimes. So an estimate of the remaining of lifetime could be used to assess model performance, given some degree of certainty.

On the other hand, an exact determination of the failure time is not required to schedule maintenance activities. 
This is especially true in circumstances where the amount of information present is actually relatively limited. 

Due to the sequential nature of the data, conventional methods of cross validation are not applicable. 

Examine three methods of model performance, WAIC, leave-one-out cross-validation and concordance index. All of these methods measure the out-of-sample predictive performance of the model, but do so through different means. As such, if the model is stable, all three should result in the same conclusion being reached about the model. Yet, if an error in model design or fit has occurred, their lack of harmony should be a sign to reevaluate the current model. 
