...
\begin{lstlisting}
functions {
  real log\_h\_t(real lifetime, real alp, real gam, real lin_pred);
  real H\_t(real lifetime, real alp, real gam, real lin_pred);
  ...
  real surv\_dens\_log(vector cens_lifetime, real alp, real gam, real lin_pred){
    real lifetime;
    real d_i;
    ...
    return d_i * log\_h\_t(lifetime, alp, gam, lin_pred) - H\_t(lifetime, alp, gam, lin_pred);
  }
}
\end{lstlisting}
...
\begin{lstlisting}
generated quantities {
  vector[N] pred_log\_h\_t;
  for(i in 1:N){
    pred_log\_h\_t[i] <- log\_h\_t(lifetime[i,2], alp, gam, beta_err1 * x_err1[i] + beta_err2 * x_err2[i] + beta_repair * x_repair[i] + beta_kWh * x_kWh[i] + beta_weather * park_weather[i]);
  }
}
\end{lstlisting}
...
generated quantities {
  vector[N] log_lik;
  for (i in 1:N){
  	log_lik[i] <- surv\_dens\_log(lifetime[i], alp, gam, beta_err1 * x_err1[i] + beta_err2 * x_err2[i] + beta_repair * x_repair[i] + beta_kWh * x_kWh[i] + beta_weather * park_weather[i]);
  }
}
\end{lstlisting}
...
functions {
  ...
  real surv\_dens\_log(vector cens_lifetime, real alp, real gam, real sig, real lin_pred){
    real lifetime;
    real d_i;
