...
The extension to shared Frailty adds an additional complication. Specifically, for each cluster a separate frailty 


% Shared Frailty Likelihood goes here:
...
This normalizing constant is interesting because in Bayesian models, it often lacks a closed form solution and has to be estimated through an alternative process. This produces a similar situation to the one found in the time-to-event models described so far in the text. When the closed form solution is not available, the methods of Bayesian model estimation become useful. Shared Frailties can be re-contextualized as Bayesian hierarchical models. However, even when closed form solutions are available, the Bayesian method of fitting may prevent the need to manually integrate a complex likelihood. 
...
The most common approach for describing the posterior distributions make use of MCMC methods. These methods generate representative samples from the posterior distribution.
