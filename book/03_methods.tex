...
Where $\delta_i = \sum^{n_i}_{j=1} d_{ij}$ or the count of events within each cluster and $\Gamma (t) = \int _{0}^{\infty }x^{t-1}\exp(-x)dx$ is the Gamma function. Taking the log of the above, we get the log likelihood:
