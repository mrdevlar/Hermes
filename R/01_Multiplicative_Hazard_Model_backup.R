...
## With different baseline hazards
alps = c(3, 2.5, 2, 4, 7)
alp = rep(alps, park_N)
gams = c(5, 7, 10, 3, 8)
gam = rep(gams, park_N)
...
# (II)
# No random effect
# r_lifetime = ((gam^alp) * (-log(U) * exp(-lin_pred)) )^(1/alp)
# r_lifetime = ((gam^alp) * (-log(U) * exp(lin_pred)) )^(1/alp)

# With random effect, note bias
r_lifetime = ((gam^alp) * (-log(U) * exp(-lin_pred) * park_Zij) )^(1/alp)
# r_lifetime = ((gam^alp) * (-log(U) * exp(lin_pred) * park_Zij) )^(1/alp)


dfc = data.frame(r_lifetime, H_t = -pweibull(r_lifetime, alp, gam, lower = FALSE, log = TRUE))
dfc %>% ggplot(aes(x = r_lifetime, y = H_t)) + geom_point()
...
functions {
  ...
  real S_t(real lifetime, real alp, real gam, real lin_pred);
  ...
  real S_t(real lifetime, real alp, real gam, real lin_pred){
    return (exp(-((gam^(-alp) * lifetime )^alp)))^exp(lin_pred);
  }
...
generated quantities {
  ...
  for(i in 1:N){
    pred_log_h_t[i] <- log_h_t(lifetime[i,2], alp, gam, beta_err1 * x_err1[i] + beta_err2 * x_err2[i] + 
        beta_repair * x_repair[i] + beta_kWh * x_kWh[i] + beta_weather * park_weather[i]);
    pred_H_t[i] <- H_t(lifetime[i,2], alp, gam, beta_err1 * x_err1[i] + beta_err2 * x_err2[i] + 
        beta_repair * x_repair[i] + beta_kWh * x_kWh[i] + beta_weather * park_weather[i]);
    pred_S_t[i] <- S_t(lifetime[i,2], alp, gam, beta_err1 * x_err1[i] + beta_err2 * x_err2[i] + 
        beta_repair * x_repair[i] + beta_kWh * x_kWh[i] + beta_weather * park_weather[i]);
  }
}
...
samp_log_h_t = extract(mhazm, pars=c("pred_log_h_t"))
samp_H_t = extract(mhazm, pars=c("pred_H_t"))
samp_S_t = extract(mhazm, pars=c("pred_S_t"))
...
lhaz = log( (mean(s_alp)/mean(s_gam)) ) + (mean(s_alp) - 1) * log( (lifetime / mean(s_gam)) ) + pred_lin_pred;


# Put it all together
df = data.frame(h_t = colMeans(samp_log_h_t$pred_log_h_t),
                lh_t = lhaz,
                H_t = colMeans(samp_H_t$pred_H_t),
                S2_t = colMeans(samp_S_t$pred_S_t),
                H2_t = -pweibull(lifetime, mean(s_alp), mean(s_gam), lower = FALSE, log = TRUE),
...
# In-sample C-Index
C_Index(df)
C_Index(df, metric = "lh_t")
# Out-of-sample C-Index
C_Index(df, lifetime = "r_lifetime")
C_Index(df, lifetime = "diff_times", forward = FALSE)



df %>% ggplot(aes(y = h_t, x= lifetime)) + geom_point()
df %>% ggplot(aes(y = H_t, x= lifetime)) + geom_point()
df %>% ggplot(aes(y = H2_t, x= lifetime)) + geom_point()
df %>% ggplot(aes(y = S2_t, x= lifetime)) + geom_point()
df %>% ggplot(aes(y = h_t, x= diff_times)) + geom_point()
